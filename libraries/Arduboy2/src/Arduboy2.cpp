...
void Arduboy2Base::ardBitmapDrawCompressed(int16_t sx, int16_t sy, const uint8_t *compBitmap, uint8_t color, uint8_t align, uint8_t mirror)
{
      ...
      if (characterPos == 0xFF){
        ...
        if (decByte && bRow < (HEIGHT / 8) && iColMod + sx < WIDTH && iColMod + sx >= 0){
          ...
          if (bRow >= 0) {
            ...
            data = getBuffer()[(bRow * WIDTH) + sx + iColMod];
            if (color) {
              data |= bitmap_data & 0xFF;
            }else {
              data &= ~(bitmap_data & 0xFF);
            }
            getBuffer()[(bRow * WIDTH) + sx + iColMod] = data;
          }

          if (yOffset && bRow < (HEIGHT / 8) - 1 && bRow > -2) {

            data = getBuffer()[((bRow + 1) * WIDTH) + sx + iColMod];
            if (color) {
              data |= ((bitmap_data >> 8) & 0xFF);
            } else {
              data &= ~(((bitmap_data >> 8) & 0xFF));
            }
            getBuffer()[((bRow + 1)*WIDTH) + sx + iColMod] = data;
          }
        }
...
void Arduboy2Base::ardBitmapDrawCompressedResized(int16_t sx, int16_t sy, const uint8_t *compBitmap, uint8_t color,uint8_t align, uint8_t mirror, float resize)
{
          ...
          for (uint8_t s = 0; s < 8 ;s++){
            ...
            if (y_LUT[aRow8+s] != 0xFF && decByte &  BIT_SHIFT[s]){
              ...
              if (row < (HEIGHT / 8)) {
                ...
                if (color) {
                  getBuffer()[(row*WIDTH) + (uint8_t)iColMod] |=   BIT_SHIFT[((uint8_t)(y_LUT[aRow8+s]+sy) % 8)];
                } else {
                  getBuffer()[(row*WIDTH) + (uint8_t)iColMod] &= ~ BIT_SHIFT[((uint8_t)(y_LUT[aRow8+s]+sy) % 8)];
                }
              }
...
void Arduboy2Base::ardBitmapDrawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, uint8_t w, uint8_t h, uint8_t color, uint8_t align, uint8_t mirror)
{
      ...
      for (uint8_t a = 0; a < loop_h; a++) {
        for (uint8_t iCol = 0; iCol < rendered_width; iCol++) {
          ...
          if(data) {
            ...
            if (yOffset != 0 && sRow < 7) {
              data = getBuffer()[ofs + WIDTH];
              if (color){
                data |= (bitmap_data >> 8) & 0xFF;
              } else{
                data &= ~((bitmap_data >> 8) & 0xFF);
              }
              getBuffer()[ofs + WIDTH] = data;
            }
          }
          ofs++;
...
void Arduboy2Base::ardBitmapDrawBitmapResized(int16_t sx, int16_t sy, const uint8_t *bitmap, uint8_t w,uint8_t h, uint8_t color,uint8_t align, uint8_t mirror, float resize)
{
